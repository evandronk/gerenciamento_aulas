...
from db.professor_db import salvar_professor as salvar_professor_db, verificar_professor_existente

class CadastrarProfessorView:
    ...
    def build(self, page: ft.Page):
        nome_input = ft.TextField(label="Nome", width=415)
        ...
        def atualizar_dropdowns():
            ...
            for dia, dropdown in self.dropdown_map.items():
                dropdown.options = [
                    ft.dropdown.Option(
                        text=f"{h} (Indisponível)" if h in [horario for d, horario in self.restricoes if d == dia] else h,
                        key=h,
                        disabled=h in [horario for d, horario in self.restricoes if d == dia]
                ...
                restricoes_lista.controls.append(
                    ft.Row([
                        ...
                        ft.IconButton(
                            ...
                            on_click=lambda e, dia=d, hora=h: remover_restricao(e, dia, hora)
                        )
                    ], alignment='start')
                )

        def salvar_professor(e):
            nome_professor = nome_input.value.strip()

            if not nome_professor:
                alerta_erro = ft.AlertDialog(
                    title=ft.Text("Erro"),
                    content=ft.Text("O nome do professor não pode estar vazio."),
            ...
            if verificar_professor_existente(nome_professor):
                alerta_erro = ft.AlertDialog(
                    title=ft.Text("Erro"),
                    content=ft.Text(f"Professor '{nome_professor}' já cadastrado anteriormente."),
                    actions=[ft.TextButton("OK", on_click=lambda e: self.fechar_dialogo(page))],
                )
                page.dialog = alerta_erro
                page.dialog.open = True
            ...
            # Transformar a lista de restrições em uma lista de objetos Horario
            lista_restricoes = [
                Horario(periodo=0, horario=[horario], dia_semana=dia)
                for dia, horario in self.restricoes
            ] if self.restricoes else None

            salvar_professor_db(nome_professor, lista_restricoes)

            alerta_sucesso = ft.AlertDialog(
                title=ft.Text("Sucesso"),
                content=ft.Text(f"O Professor '{nome_professor}' foi cadastrado com sucesso."),
                actions=[ft.TextButton("OK", on_click=lambda e: self.fechar_dialogo(page))],
            )
        ...
        restricoes_lista = ft.ListView([], height=200, auto_scroll=True)

        return ft.Container(
            content=ft.Column([
                ft.Text("Cadastrar Professor", size=30),
                nome_input,
                ft.Row(
                    [
                        # Coluna de dropdowns
